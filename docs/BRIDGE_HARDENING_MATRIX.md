...
| Outbound attachments | Valid `BRIDGE_ATTACH` stripped or ignored by prompt heuristics | Valid bridge directives are now explicit transport handoffs with live marked image-attachment smoke evidence, and trusted-gate evidence can surface missing or failed trusted-chat replies | Add `/codex smoke attachment` evidence from the trusted Messages chat |
