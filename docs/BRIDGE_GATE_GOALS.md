...
- Architecture gate:
  - The backend now has a JSON-RPC responder channel for app-server callbacks. The CLI and Messages command surfaces both have app-server callback smokes; the current live runtime proof is blocked before bridge callback handling because Codex reports `request_user_input is unavailable in Default mode`.
- Current status:
...
- Current status:
  ...
  - Messages-launched `/codex smoke ...` commands persist their start/result evidence as `messages-*` live smoke records. Long-running generated-image and app-server-callback smokes overwrite the initial pending record with final pass/block/fail evidence, thread id, and turn id when the active app-server job completes.
