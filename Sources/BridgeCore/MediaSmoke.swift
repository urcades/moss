...
public struct OutboundImageSmokeRequest: Equatable, Sendable {
    public var marker: String
    public var mediaRef: RecentMediaRef
    public var request: PromptRequest
...
public func buildOutboundImageSmokeRequest(
    recipient: String,
    service: String,
    recentMediaRefs: [RecentMediaRef],
    now: Date = Date(),
    marker: String = "CODEXMSGCTL_SMOKE_OUTBOUND_IMAGE_\(UUID().uuidString)"
) throws -> OutboundImageSmokeRequest {
    guard let ref = recentMediaRefs.reversed().first(where: { mediaRef in
        mediaRef.direction == "outbound" &&
            mediaRef.kind == "image" &&
            mediaRef.handleId == recipient &&
            mediaRef.service == service &&
            mediaRef.exists &&
        ...
        throw StoreError.validation("Smoke outbound-image-check failed: no usable app-server-compatible recent outbound image for \(recipient) via \(service). Send or generate an image through the bridge first, then rerun this smoke.")
    }
    let nowText = DateCodec.iso(now)
    let batch = PendingBatch(
        handleId: recipient,
